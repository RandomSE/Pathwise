...
class TestCarDiagnosticsLogger(unittest.TestCase):
    ...
    def test_turn_phase_uses_entry_travel(self):
        self.logger.begin_round(1)
        car = _StubCar(vertical=False, direction=1)
        car._turn_phase = "turning"
        car._turn_exit = (2, -1, True)
        car._turn_entry_vertical = True
        car._turn_entry_direction = -1
        vertical, direction = self.logger._effective_travel(car)
    ...
    def test_hub_offset_exception_handled(self):
        car = MagicMock()
        car.rect = Rect(0, 0, 10, 10)
        car._hub_travel_offset.side_effect = RuntimeError("boom")
        car._turn_hub = (1, 2)
        car._turn_exit = None
        car.vertical = False
        car.direction = 1
        snap = self.logger._car_snapshot(car, dx=0, dy=0, along_travel=0, moved=0)
        self.assertIsNone(snap["hub_travel_offset"])
